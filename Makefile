...
ifeq ($(OSPRETTY), macOS)
    EXCL := gl_core_4_1.c $(patsubst %_metal.mm, %.cpp, $(wildcard *_metal.mm)) $(wildcard *_opengl.cpp) $(wildcard *_d3d11.cpp)
else
    ifeq ($(OSPRETTY), Windows)
        EXCL := gl_core_4_1.c $(wildcard *_opengl.cpp)
    else
        EXCL := $(wildcard *_d3d11.cpp)
    endif
endif
